class Slider {
  constructor(sliderSelector, gallerySelector, slidesPerView) {
    //хранит объект слайдера (всего)
    this.slider = document.querySelector(sliderSelector);
    //хранит объект галереи (только контент)
    this.gallery = document.querySelector(gallerySelector);
    //количество слайдов на странице изначально
    this.slidesPerView = slidesPerView;
    //количество
    this.slidesCount = this.gallery.children.length;
    //расстояние между контентом
    this.galleryGap = 2;
    //проверка на отсутсвие параметров
    if (!this.slider || !this.gallery || !this.isSliderValid()) {
      console.error(
        "Slider or gallery element not found, or slider is invalid."
      );
      return;
    }
  ...
  isSliderValid() {
    return Array.from(this.gallery.children).every((element) =>
  ...
  controlDots() {
    if (
      !Array.from(this.slider.children).some((child) =>
        child.className.includes("dot")
      )
    ) {
      ...
      for (
        let pos = 0;
        pos < this.slidesCount - (this.slidesPerView - 1);
        pos++
      ) {
  ...
  updateSliderPosition() {
    const translateXValue =
      -this.currentSlide * (this.slideWidth + this.galleryGap);
    this.gallery.style.transform = `translateX(${translateXValue}px)`;
  }
}

document.addEventListener("DOMContentLoaded", () => {
  const slider = new Slider(".slider", ".gallery", 3);
});
