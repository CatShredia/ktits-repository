...
public class UnitTests
{
    ...
    // ! =============== Task 11: Cup class Test ===============
    // Опишите класс «стакан». Поля класса: высота стакана, диаметр дна стакана. 
    // Предполагается, что стакан прямой (представляет собой цилиндр). Методы: определить объем стакана, определить, 
    ...
    [Test]
    public void Task12VolumeTest()
    {
        var result = TestConsoleProject.;
        var expected = Math.Round((4.0 / 3.0) * Math.PI * Math.Pow(100, 3), 0);
        Assert.That(result, Is.EqualTo(expected));
    }
}
